...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${PYTHON_BACKEND_URL}/regular_calendar/reject_invitation`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.error || 'Failed to reject invitation' },
    ...
    console.error('Error forwarding reject invitation request to Python backend:', error);
    return NextResponse.json(
      { 
