-- Safe course schema - only creates tables if they don't exist
...
CREATE POLICY "Anyone can view course lessons" ON public.lessons FOR SELECT USING (is_published = true);
...
SELECT 'Programming', 'Learn programming languages and frameworks', '#3B82F6'
WHERE NOT EXISTS (SELECT 1 FROM public.categories WHERE name = 'Programming');

INSERT INTO public.categories (name, description, color) 
SELECT 'AI & Machine Learning', 'Artificial Intelligence and ML courses', '#8B5CF6'
WHERE NOT EXISTS (SELECT 1 FROM public.categories WHERE name = 'AI & Machine Learning');

INSERT INTO public.categories (name, description, color) 
SELECT 'Web Development', 'Frontend and backend web development', '#10B981'
